...
import { Search, Calendar, Tag, FileText, CornerDownLeft, X } from 'lucide-react';
...
export const SearchModal: React.FC<SearchModalProps> = ({
  ...
  return (
    <div
      className={`fixed inset-0 z-50 flex items-start justify-center pt-16 sm:pt-24 px-4 transition-all duration-200 animate-in fade-in ${
    ...
    >
      <div
        ...
        className={`w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[80vh] border transition-all duration-200 ${
          isDark
      ...
      >
        ...
        >
          <Search className={`w-5 h-5 shrink-0 ${isDark ? 'text-slate-400' : 'text-slate-500'}`} />
          <input
            ref={inputRef}
            type="text"
            value={query}
            ...
            className={`w-full bg-transparent border-none text-sm font-sans focus:outline-none ${
          ...
          <kbd
            className={`hidden sm:inline-block px-2 py-0.5 text-[10px] font-mono font-medium rounded border shadow-xs ${
              isDark
        ...
        <div
          className={`px-4 py-2 border-b flex items-center justify-between gap-2 overflow-x-auto text-xs ${
            ...
            <div className="flex items-center gap-1 overflow-x-auto shrink-0 max-w-[45%] no-scrollbar">
              {allTags.slice(0, 4).map((tag) => (
                <button
                  ...
                  className={`px-2 py-0.5 rounded-md font-mono text-[10px] shrink-0 transition-colors ${
                    isDark
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1">
          {filteredNotes.length === 0 ? (
            <div className="py-12 text-center text-xs font-sans space-y-1">
              <FileText className={`w-8 h-8 mx-auto ${isDark ? 'text-slate-700' : 'text-slate-300'}`} />
              ...
              return (
                <div
                  ...
                  className={`p-3 rounded-xl cursor-pointer transition-all flex items-start justify-between gap-3 border ${
                    isSelected
                      ? isDark
                        ? 'bg-blue-950/40 border-blue-800/70 text-slate-100 shadow-sm'

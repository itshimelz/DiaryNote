...
export async function initDatabase(): Promise<{
  ...
  try {
    ...
    // Run compactDatabase on startup to prune legacy storage and orphaned records
    ...
    console.error('Failed to initialize IndexedDB database, falling back to default:', error);
    return {
      notes: SAMPLE_NOTES,
...
export async function loadNotesFromDB(): Promise<Note[]> {
  try {
    const rawNotes = await db.notes.toArray();
    return await Promise.all(rawNotes.map(prepareNoteForMemory));
  } catch (err) {

...
| **Phase 3 (Task 14)** | `src/components/CanvasControls.tsx` | Settings popover, update checker toggle | Added "Check Updates on Launch" network transparency toggle bound to `AppSettings`. | User can opt out of GitHub release checks on app startup. | Toggle update check switch in settings popover; verify setting persists. |
